...
{
	int selection = [ radialsMatrix selectedRow ] ;
...
- (Boolean)generateRadials:(NSMutableString*)string eval:(Spreadsheet*)eval
{
	...
	if ( radius < 0.0001 ) {
		[ [ NSAlert alertWithMessageText:@"Wire radius specified for radials is too small." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nRadials are not generated.\n\nPlease correct in the Environment:Radials sheet.\n" ] runModal ] ;
		return NO ;
	}
	
	radials = [ eval intValueForObject:[ radialsParamMatrix cellAtRow:2 column:0 ] ] ;
	if ( radials < 1 ) {
		[ [ NSAlert alertWithMessageText:@"Number of radials fewer than one??" defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\n\nRadials are not generated.\n\nYou need at least one element!\n\nPlease correct in the Environment:Radials sheet.\n" ] runModal ] ;
		return NO ;
	}
	
	length = [ eval doubleValueForObject:[ radialsParamMatrix cellAtRow:0 column:0 ] ] ;
	if ( length < radius*10 ) {
		[ [ NSAlert alertWithMessageText:@"The length of a radial wire is too short." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\n\nRadials are not generated.\n\nThe length needs to be at least 10 times longer than the radius of the wire.\n\nPlease correct in the Environment:Radials sheet.\n" ] runModal ] ;
		return NO ;
	}
...
- (NSMutableDictionary*)makeRadials 
{
	...
	[ dict setObject:[ NSNumber numberWithInt:[ radialsMatrix selectedRow ] ] forKey:@"has radials" ] ;
