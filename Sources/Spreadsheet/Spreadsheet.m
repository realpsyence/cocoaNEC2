...
- (id)initWithGlobals:(GlobalContext*)glob number:(int)num untitled:(Boolean)isUntitled
{
	...
	if ( self ) {
		...
		plist = nil ;
		sourcePath = nil ;
		if ( [ NSBundle loadNibNamed:@"Spreadsheet" owner:self ] == NO ) return nil ; 
...
- (Boolean)outputHollerith:(NSString*)path
{
	...
	Exception *exception ;
	NSArray *array ;
	int i, j, wires, tag, count, step, steps, total ;
	...
	if ( total <= 0 ) {
		[ [ NSAlert alertWithMessageText:@"No Geometry Element??" defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nThere is no wire in your model.\n" ] runModal ] ;
		return NO ;
	}
	...
	if ( total <= 0 ) {
		[ [ NSAlert alertWithMessageText:@"No Excitation??" defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nYou need to add an exitation (feed point) to at least one of the wires.\n" ] runModal ] ;
		return NO ;
	}
...
- (Boolean)createNCProgram
{
	...
	NSString *dateString = [ [ NSDate date ] descriptionWithCalendarFormat:@"%Y-%m-%d %H:%M" timeZone:nil locale:nil ] ;
