...
- (Boolean)ncCode:(NSMutableString*)code eval:(Spreadsheet*)client networkRow:(int)row
{
	...
	if ( crossed ) {
		...
		[ code appendFormat:@"%s,", [ [ [ tlMatrix cellAtRow:0 column:0 ] stringValue ] UTF8String ] ] ;			// z0
			...
			printf( "length (%s)\n", [ [ [ tlMatrix cellAtRow:1 column:0 ] stringValue ] UTF8String ] ) ;

	[ code appendFormat:@"%s,", [ [ [ tlMatrix cellAtRow:1 column:0 ] stringValue ] UTF8String ] ] ;				// length
...
- (NSString*)networkCard:(Expression*)e spreadsheet:(Spreadsheet*)client networkRow:(int)row
{
	...
	return [ NSString stringWithFormat:@"TL%3d%5d%5d%5d%10s%10s%10s%10s%10s%10s", tag1, seg1, tag2, seg2, z0, length, mho1real, mho1imag, mho2real, mho2imag ] ;
}
...
- (NSMutableDictionary*)makePlistDictionary
{
	...
	[ plist setValue:[ NSNumber numberWithInt:[ tlLocation1Matrix selectedRow ] ] forKey:@"matrix1" ] ;
	[ plist setValue:[ NSNumber numberWithInt:[ tlLocation2Matrix selectedRow ] ] forKey:@"matrix2" ] ;
