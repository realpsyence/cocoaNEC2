...
- (id)initWithDelegate:(id)client withType:(int)type
{
	self = [ super init ] ;
	if ( self ) {
		delegate = client ;
		...
		opened = ignore = NO ;
		to = from = comment = nil ;
		if ( [ NSBundle loadNibNamed:@"TwoPort" owner:self ] == NO ) return nil ; 
	}
	return self ;
}
...
- (void)awakeFromNib
{
	...
	[ typeTab selectTabViewItemWithIdentifier:[ NSString stringWithFormat:@"%d", selectedType ] ] ;
