...
- (Boolean)parse
{
	...
	NCObject *ncObject ;
	NCExpression *expr ;
	int argumentsUsedInFunction, argType ;
...
- (NCObject*)getArrayReference:(RuntimeStack*)inStack	
{
	...
	if ( index < 0 || index >= [ exprValue arrayDimension ] ) {
		[ [ NSAlert alertWithMessageText:@"Runtime error." defaultButton:@"OK" alternateButton:nil otherButton:nil 
				informativeTextWithFormat:@"\nThe array '%s' is deferenced with an out of bounds index %d.\n\nUsing array[0].\n\n", [ [ [ primary ncObject ] name ] UTF8String ], index  ] runModal ] ;	
