		...
		while ( [ remainder length ] > 0 ) {
			...
			if ( [ remainder characterAtIndex:mark ] == '"' ) {
				...
				if ( [ includeArray count ] > 256 ) {
					[ [ NSAlert alertWithMessageText:@"Include file nesting too deep!." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"Include files nesting exceeded 256 (do you really mean to do this?)." ] runModal ] ;
					return nil ;
				}
				for ( j = 0; j < [ includeArray count ]; j++ ) {
					if ( [ pathName isEqualToString:[ includeArray objectAtIndex:j ] ] ) {
						[ [ NSAlert alertWithMessageText:@"Include file nesting loops!." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"Include files are including each other in an infinite loop." ] runModal ] ;
						return nil ;
					}
				}
				...
				if ( included == nil ) {
					[ [ NSAlert alertWithMessageText:@"Included file not found." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"" ] runModal ] ;
					return nil ;
				}
...
- (id)initWithString:(NSString*)str documentNumber:(int)inDocumentNumber
{
	...
	if ( self ) {
		...
		preParser = [ [ NCPreParser alloc ] initWithSource:str ] ;
