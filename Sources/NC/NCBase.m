...
{
	int i, j, t, errorCount, line, parseErrorIndex, parseErrorLine, firstErrorPosition ;
	const char *input ;
	char buf[182], buf8[182], *c ;					// 0.74
	int lineLocation[MAXLINES] ;
...
- (Boolean)outputHollerithToFile:(NSString*)filePath
{
	...
	if ( count == 0 ) {
		[ [ NSAlert alertWithMessageText:@"Hollerith card deck not created." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nError found in the interpreter, Card deck is not created.\n" ] runModal ] ;	
		return NO ;
	}
...
- (void)reportRuntimeErrors
{
	...
	if ( errCount > 0 ) {
		[ [ NSAlert alertWithMessageText:@"Runtime Error." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:[ stack.errors objectAtIndex:errCount-1 ] ] runModal ] ;
	}
}
