...
//  Copyright 2007, 2009 Kok Chen. W7AY. All rights reserved.
//

#import "NCSystem.h"
#import "ApplicationDelegate.h"
#import "cocoaNECTypes.h"
#import "NC.h"
...
- (void)makeSystem:(Boolean)asSpreadsheet
{
	...
	short transmissionAtSegmentsPrototype[] = { ELEMENTTYPE, INTTYPE, ELEMENTTYPE, INTTYPE, REALTYPE, 0 } ;
	...
	[ symbolTable addObject:[ [ NCFunctionObject alloc ] initWithSystem:"exp" type:REALTYPE selector:@selector(sqrt:prototype:) argPrototypes:realPrototype ] ] ;
...
- (void)nec4Insulate:(NCWire*)element insulationRadius:(double)radius permittivity:(double)permittivity conductivity:(double)conductive
{
	...
	if ( engine == knec2cEngine ) {
		[ [ NSAlert alertWithMessageText:@"Adding insulation sheath to wire?" defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nThe IS (insulation sheath) card only works with NEC-4.\n\nThe card will not be generated." ] runModal ] ;	
		return ;
	}
...
- (NCValue*)insulate:(NSArray*)args prototype:(NSArray*)prototype
{
	...
	if ( param ) {
		element = (NCWire*)[ param elementValue ] ;
		if ( element == nil ) {
			[ [ NSAlert alertWithMessageText:@"Argument error in insulate function." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nThe first argument of the insulate function should be a wire element." ] runModal ] ;	
		}
		else {
			permittivity = doubleValue( args, 1 ) ;
...
//	v0.81b
- (NCValue*)coax:(NSArray*)args prototype:(NSArray*)prototype y1r:(double)y1r y1i:(double)y1i y2r:(double)y2r y2i:(double)y2i crossed:(Boolean)crossed shield:(Boolean)hasShield
{
	NCWire *element1, *element2 ;
	NCCoax *coax ;
	NCCoaxCable *cable ;
		...
		if ( param ) {
			...
			if ( param ) {
				...
				if ( coax != nil ) {
					...
					p.end1 = p.end2 = ( hasShield ) ? 0 : 10 ;
					if ( crossed ) p.end2 += 100 ;
					p.y1r = y1r ;
					p.y1i = y1i ;
					p.y2r = y2r ;
					p.y2i = y2i ;
...
{
	return [ self coax:args prototype:prototype y1r:0 y1i:0 y2r:0 y2i:0 crossed:NO shield:NO ] ;
}

//	v0.78, v0.81
- (NCValue*)crossedCoax:(NSArray*)args prototype:(NSArray*)prototype
{
	return [ self coax:args prototype:prototype y1r:0 y1i:0 y2r:0 y2i:0 crossed:YES shield:NO ] ;
}

//	v0.78, v0.81
- (NCValue*)terminatedCoax:(NSArray*)args prototype:(NSArray*)prototype
{
	return [ self coax:args prototype:prototype y1r:doubleValue( args, 3 ) y1i:doubleValue( args, 4 ) y2r:doubleValue( args, 5 ) y2i:doubleValue( args, 6 ) crossed:NO shield:NO ] ;
}

//	v0.78, v0.81
- (NCValue*)crossedTerminatedCoax:(NSArray*)args prototype:(NSArray*)prototype
{
	return [ self coax:args prototype:prototype y1r:doubleValue( args, 3 ) y1i:doubleValue( args, 4 ) y2r:doubleValue( args, 5 ) y2i:doubleValue( args, 6 ) crossed:YES shield:NO ] ;
}

//	v0.81
- (NCValue*)coaxWithShield:(NSArray*)args prototype:(NSArray*)prototype
{
	return [ self coax:args prototype:prototype y1r:0 y1i:0 y2r:0 y2i:0 crossed:NO shield:YES ] ;
}

//	v0.81
- (NCValue*)crossedCoaxWithShield:(NSArray*)args prototype:(NSArray*)prototype
{
	return [ self coax:args prototype:prototype y1r:0 y1i:0 y2r:0 y2i:0 crossed:YES shield:YES ] ;
}

//	v0.81
- (NCValue*)terminatedCoaxWithShield:(NSArray*)args prototype:(NSArray*)prototype
{
	return [ self coax:args prototype:prototype y1r:doubleValue( args, 3 ) y1i:doubleValue( args, 4 ) y2r:doubleValue( args, 5 ) y2i:doubleValue( args, 6 ) crossed:NO shield:YES ] ;
}

//	v0.81
- (NCValue*)crossedTerminatedCoaxWithShield:(NSArray*)args prototype:(NSArray*)prototype
{
	return [ self coax:args prototype:prototype y1r:doubleValue( args, 3 ) y1i:doubleValue( args, 4 ) y2r:doubleValue( args, 5 ) y2i:doubleValue( args, 6 ) crossed:YES shield:YES ] ;
}
...
- (NCValue*)print:(NSArray*)args prototype:(NSArray*)prototype
{
	...
	if ( count == 1 ) {
		...
		resultString = [ NSString stringWithString:@"" ] ;
		remainingfmt = fmt ;
...
- (NCElement*)wirev:(NSArray*)args prototype:(NSArray*)prototype
{
	...
	if ( [ value type ] != TRANSFORMTYPE ) {
		[ [ NSAlert alertWithMessageText:@"Bad argument in function call." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nThe first argument should be a transform variable.  No transform is executed.\n" ] runModal ] ;
		transform = nil ;
	}
	else {
...
- (NCValue*)matrixElement:(NSArray*)args prototype:(NSArray*)prototype
{
	...
	if ( i < 0 || i > 2 || j < 0 || j > 2 ) {
		[ [ NSAlert alertWithMessageText:@"Bad argument in function call." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"Index for matrix element must be 0, 1 or 2.\n" ] runModal ] ;
		return [ NCValue valueWithDouble:0 ] ;
	}
	transform = [ value transformValue ] ;
