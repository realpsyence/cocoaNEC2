...
{	
	...
	if ( segmentLength < radius ) [ [ NSAlert alertWithMessageText:@"TaperedWire warning." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nSegment length is less than the wire radius.  This could lead to inaccurate results.\n" ] runModal ] ;

	return [ [ NSString alloc ] initWithFormat:@"GW%3d%5d%10s%10s%10s%10s%10s%10s%10s", tagn, segs, dtos(sx1), dtos(sy1), dtos(sz1), dtos(sx2), dtos(sy2), dtos(sz2), dtos(radius) ] ;
}
