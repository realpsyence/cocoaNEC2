...
- (Boolean)declarationList
{
			...
			if ( token != RPAREN ) {
				...
				while ( 1 ) {
					if ( token != INTTYPE && token != REALTYPE && token != ELEMENTTYPE && token != COAXTYPE && token != VECTORTYPE && token != TRANSFORMTYPE ) {	//  v0.81b
						token = [ parser setError:[ NSString stringWithString:@"missing type declaration for argument variable (should be int, real or element)" ] flush:YES ] ;
						return NO ;
					}
					typeOfArg = token ;
					isReference = NO ;
					...
					if ( token != ALPHA ) {
						token = [ parser setError:[ NSString stringWithFormat:@"bad identifier in argument list", token ] flush:YES ] ;
						return NO ;
					}
					...
					if ( token != COMMA ) {
						token = [ parser setError:[ NSString stringWithString:@"expect arguments that are separated by commas" ] flush:YES ] ;
						return NO ;
					}
					token = [ parser nextToken ] ;
				}
	...
	if ( p == nil || [ p op ] != RETURNSTATEMENT ) {
		if ( !( type == MODELBLOCK || type == CONTROLBLOCK || type == VOIDTYPE ) ) {
			token = [ parser setErrorInPreviousLine:[ NSString stringWithString:@"missing return statement" ] flush:NO ] ;
			return NO ;
		}
	}
	return success ;
...
{
	int i, argCount, localVariablesCount ;
