...
- (NSDictionary*)makeFontAttributeForSize:(float)size
{
	...
	verdana = [ NSFont fontWithName: @"Verdana" size:size ] ;
...
- (id)init
{
	...
	self = [ super init ] ;
	if ( self ) {
		if ( [ NSBundle loadNibNamed:@"Output" owner:self ] == NO ) return nil ; 
...
- (void)displayContext:(int)index
{
	...
	double averageGain ;
	int i, count, elementCount, j, feedpointArrays, engine, whichTab ;
	...
	[ self appendSummary:@"---------  " ] ;
	[ self appendSummary:[ [ NSDate date ] descriptionWithCalendarFormat:@"%Y-%m-%d %H:%M" timeZone:nil locale:nil ] ] ;
	...
	int length = [ [ summary string ] length ] ;
	int remove = 0 ;
	if ( length > 10000 ) {
		remove = length-10000 ;
...
- (void)useAsReference
{
	if ( currentContext == nil ) {
		[ [ NSAlert alertWithMessageText:@"No output to set as Reference." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nYou can only set the Reference to an opened output model.\n" ] runModal ] ;
		return ;
	}
...
//	v0.68
- (void)setPlotPolarization:(int)pol
{
...
- (void)polarizationChanged:(int)pol
{
...
- (id)comboBox:(NSComboBox*)comboBox objectValueForItemAtIndex:(intType)index
{
	...
	if ( comboBox == modelList ) {
		...
		int numberOfContexts = [ contexts count ] ;
...
{
	int count, which, result ;
	NSPrintInfo *printInfo ;
	
	if ( ![ window isVisible ] ) {
		[ [ NSAlert alertWithMessageText:@"Output window is not open." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nTo print an output view, the output window must be open and tabbed to the output view that you wish to print.\n\nSelect Output Viewer in the Window Menu.\n" ] runModal ] ;
		return ;
	}
	count = [ contexts count ] ;
	if ( count <= 0 ) {
		[ [ NSAlert alertWithMessageText:@"Output window is empty." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nThere is no antenna model present in the output window.\n" ] runModal ] ;
		return ;
	}
	...
	case kCardsTab:
		...
		break ;
	case kNECListTab:
		result = [ [ NSAlert alertWithMessageText:@"Warning: large output." defaultButton:@"Cancel" alternateButton:@"Print" otherButton:nil informativeTextWithFormat:@"\nYou are asking for a large file to be printed.  Do you really want to print the NEC-2 output listing?.\n" ] runModal ] ;
		if ( result == 0 && lineprinterContainer ) [ lineprinterContainer printWithInfo:printInfo output:self ] ;
		/*
		...
		*/
		break ;
	default:
		[ [ NSAlert alertWithMessageText:@"Not implemented." defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:@"\nPrinting of this tab view is not (yet) implemented.\n" ] runModal ] ;
		break ;
	}
}
...
{
	int index, indexToRemove, n, numberOfContexts = [ contexts count ] ;
	OutputContext *context ;
