...
- (void)refreshCurrents:(int)type azimuth:(float)az elevation:(float)el zoom:(float)zoomFactor options:(GeometryOptions*)options
{
	currentType = type ;
	azAngle = az ;
	elAngle = el ;
...
//	v0.78
- (void)drawGeometryType:(int)type color:(NSColor**)colorp scale:(NSAffineTransform*)scale
{
	NSBezierPath *path ;
	NSArray *geometryArray ;
	NSColor *color ;
...
- (OutputGeometryElement*)elementWithTag:(int)tag
{
	int i, n ;
	NSArray *geometryArray ;
	OutputGeometryElement *element ;
...
- (void)drawFeedpoints:(NSAffineTransform*)scale
{
	...
	FeedpointInfo *feed ;
	int i, j, feeds, tag, tags, count, segmentOffset, relativeSegment ;
...
- (void)drawRect:(NSRect)rect
{
	...
	if ( isScreen ) {
		...
		fontAttributes = [ NSDictionary dictionaryWithObject:[ NSFont fontWithName: @"Lucida Grande" size: 11.8 ] forKey:NSFontAttributeName ] ;
	}
