...
- (void)drawRect:(NSRect)rect 
{
	...
	if ( isScreen || [ [ [ NSApp delegate ] output ] drawBorders ] ) {
		...
		[ frame lineToPoint:NSMakePoint( freqStrip.origin.x, bounds.origin.y ) ] ;
		[ frame lineToPoint:NSMakePoint( bounds.size.width, bounds.origin.y ) ] ;
	...
	if ( rxf != nil ) {	
		...
		captionAttributes = [ NSDictionary dictionaryWithObject:[ NSFont fontWithName: @"Lucida Grande" size:fontSize ] forKey:NSFontAttributeName ] ;
...
- (void)setCaptionWithRXF:(RXF*)rp z0:(float)z plotType:(int)n mainColor:(NSColor*)mainc altColor:(NSColor*)altc
{
	rxf = rp ;
	z0 = z ;
	plotType = n ;
