...
{
	[ p arc:x y:y r:r*0.8 t0:0 t1:3.1415926*2 ] ;
	[ p fill ] ;
}
...
{
	[ p arc:x y:y r:r*1.2 t0:0 t1:3.1415926*2 ] ;
	[ p fill ] ;
	//  hole in donut
	[ p save ] ;
	[ p setRGB:donutHoleColor ] ;
	[ p arc:x y:y r:r*0.55 t0:0 t1:3.1415926*2 ] ;
	[ p fill ] ;
	[ p restore ] ;	// restore RGB color
}

- (int)setCacheToClosestFrequency:(int)feedpointMenuIndex previousCache:(FeedpointCache*)previous
{
