...
- (void)drawCaptions:(PDFConsumer*)p
{
	...
	[ p save ] ;
		...
		[ self setRGBColor:0.5 g:0.0 b:0.0 context:p ] ;
		[ p setfont:"Helvetica" size:8.0 ] ;
		
		[ p save ] ;
			[ p rotate:3.1415926/2 ] ;		// for text
			RLabel( p, 9.97, "10" ) ;
			RLabel( p, 4.98, "5.0" ) ;
			RLabel( p, 1.98, "2.0" ) ;
			RLabel( p, 0.98, "1.0" ) ;
...
{
	float u, v, r, f ;
	int i, j, k, k0, kn, samples, frequencyIndex ;
	char s[64] ;
	NSPoint q, drawn ;
	RXF *rxf ;
	CubicUV *interpolate ;
    ...
    [ p save ] ;
		...
		// impedance captions
		[ p setfont:"Helvetica" size: ( isScreen ) ? 11.0*adjustedScale : 9.2 ] ;	
...
- (Boolean)mouseDownInner:(NSEvent*)event
{
	...
	uv = [ self convertPoint:[ event locationInWindow ] fromView:nil ] ;	
...
- (void)drawRect:(NSRect)rect 
{
	...
	if ( [ NSGraphicsContext currentContextDrawingToScreen ] ) {
		origin = [ self convertPoint:NSMakePoint( 0, 0 ) toView:nil ] ;			//  v0.70 offset of view origin from context (window)
		//  use screen graphics port for display
		width = bounds.size.width ;
		height = bounds.size.height ;
		center = NSMakePoint( width*0.5 + origin.x, height*0.5 + origin.y ) ;	//  note: this is center of view in window coordinate
		graphicsContext = [ NSGraphicsContext graphicsContextWithWindow:[ self window ] ] ;
		isScreen = YES ;
	}
	else {	
		//  for printing, width and height are passed in to -drawRect
...
static scramble[16] = { 2, 1, 4, 5, 0, 6, 3, 2, 1, 3, 6, 0, 5, 4, 2, 1 } ;
