...
#else 
int necmain() ;
int necExtendedDouble( char *inputFilename, char *outputFileName, int processes ) 
#endif
{
	int errorCode = 0 ;
		...
		if (forked > 0 ) {
			while (procs) {
				...
				for (i= 0 ; i<nrprocs ; i++) if (fprocs[i]->fpipe[0]>= 0 && FD_ISSET(fprocs[i]->fpipe[0], &fd_read)){
					...
					r =read(fprocs[i]->fpipe[0], fprocs[i]->buf+fprocs[i]->buflen, 4096) ;
					if (r>0 )
						fprocs[i]->buflen+=r ;
					else {
	...
	int ib11, int ic11, int id11 )
{
	...
	do {
		...
		if ( ifar != -1 ) {
			...
			if ( BuildForSnowLeopard && iplp1 != 3 && ifar != 1 && inor == 0 ) gcd_rdpat() ; else rdpat() ;
		}
	 ...
	 int iptag, int iptagf, int iptagt, int iptaq, int iptaqf, int iptaqt)
{
	...
	if ( n != 0 ) {
		...
		for ( i = 0 ; i < n ; i++ ) {
			...
			if ( ( nload != 0 ) && ( fabs( crealx( zarray[i]) ) >= 1.e-20 ) ) ploss += 0.5*cmag*cmag*crealx( zarray[i] )*si[i] ;
		...
		if ( ipr <= 10000 ) {			// 0.47 bug when extended thin wire kernel used with patches
			if ( ipr < 0 ) {
				...
				if ( -icon1[iprx] != jx ) ind1 = 2 ;
					...
					if ( ( xi < 0.999999 ) || ( fabs( bi[iprx]/b-1.0 ) > 1.e-6 ) ) ind1 = 2 ; else ind1 = 0 ;
				} /* if ( -icon1[iprx] != jx ) */
		...
		if ( ipr <= 10000 ) {
			if ( ipr < 0 ) {
				...
				if ( -icon2[iprx] != jx ) ind2 = 2 ;
					...
					xi = fabs( cabj*cab[iprx] + sabj*sab[iprx] + salpj*salp[iprx] ) ;
					if ( ( xi < 0.999999 ) || ( fabs(bi[iprx]/b-1.) > 1.e-6 ) ) ind2 = 2 ; else ind2 = 0 ;
				} /* if ( -icon1[iprx] != jx ) */
			} /* if ( ipr < 0 ) */
			else {
				iprx = ipr-1;
    ...
    doubletype zpkx, doubletype *sgr, doubletype *sgi )
{
  ...
  doubletype z, ze, s, ep, zend, dz = 0., zp, dzot= 0., t00r, g1r, g5r, t00i;
  doubletype g1i, g5i, t01r, g3r, t01i, g3i, t10r, t10i, te1i, te1r, t02r ;
...
static void intx( doubletype el1, doubletype el2, doubletype b, int ij, doubletype *sgr, doubletype *sgi, int ijaa, doubletype zpka, doubletype rkba )
{
	...
	int nx = 1, nma = 65536, nts = 4 ;
	int flag = TRUE ;
	doubletype z, s, ze, fnm, ep, zend, fns, dz = 0.0, zp, dzot= 0., t00r, g1r, g5r, t00i ;
	doubletype g1i, g5i, t01r, g3r, t01i, g3i, t10r, t10i, te1i, te1r, t02r ;
	doubletype g2r, g4r, t02i, g2i, g4i, t11r, t11i, t20r, t20i, te2i, te2r ;
	doubletype rx = 1.0e-4 ;
    ...
    doubletype *zlr, doubletype *zli, doubletype *zlc )
{
  ...
  while( TRUE )
  {
	...
	case 6:
      ...
      if (( fabs( crealx( zarray[i]))+ fabs( cimagx( zarray[i]))) > 1.0e-20 )
	iwarn =TRUE;
      zarray[i] += zt;
	...
	*ez =CPLX_00 ;
	...
	if ( n != 0 ) {
		for ( i = 0 ; i < n ; i++ ) {
			...
			if ( fabs( zp) > 0.5001* si[i] ) continue;
				...
				if ( ipr <= 10000 ) {		// v0.47
					...
					if ( ipr < 0 ) {
						...
						if ( -icon1[iprx] != ix ) ind1 = 2 ;
							...
							xi= fabs( cabj* cab[iprx]+ sabj* sab[iprx]+ salpj* salp[iprx]) ;
							if ( (xi < 0.999999) || (fabs(bi[iprx]/b-1.) > 1.0e-6) ) ind1 = 2 ; else ind1 = 0 ;
						}
					} /* if ( ipr < 0 ) */
					else
						if ( ipr == 0 ) ind1=1;
						...
						if ( ipr == 0 ) ind2=1;
							...
							if ( ipr != ix ) {
								if ( icon1[iprx] != ix ) ind2 = 2 ;
									...
									xi= fabs( cabj* cab[iprx]+ sabj* sab[iprx]+ salpj* salp[iprx]) ;
									if ( (xi < 0.999999) || (fabs(bi[iprx]/b-1.) > 1.0e-6) ) ind2 = 2 ; else ind2= 0 ;
								}
							} /* if ( ipr != (i+1 ) ) */
							else {
								if ( cabj* cabj+ sabj* sabj > 1.0e-8 ) ind1 = 2 ; else ind1 = 0 ;
			...
			if ( ipr <= 10000 ) {			//  v0.47
				if ( ipr < 0 ) {
					...
					if ( -icon1[ipr-1] != jp1 ) ind1 = 2 ;
						...
						if ( ( xi < 0.999999 ) || ( fabs(bi[ipr]/b-1.) > 1.0e-6 ) ) ind1 = 2 ; else ind1 = 0 ;
					}
				}  /* if ( ipr < 0 ) */
				else {
					if ( ipr == 0 ) ind1 = 1 ;
						...
						if ( ipr != j ) {
							if ( icon2[ipr] != jp1 ) ind1 = 2 ;
								...
								xi = fabs( cabj* cab[ipr]+ sabj* sab[ipr]+ salpj* salp[ipr] ) ;
								if ( ( xi < 0.999999 ) || ( fabs(bi[ipr]/b-1.0 ) > 1.0e-6 ) ) ind1 = 2 ; else ind1 = 0 ;
							}
						} /* if ( ipr != j ) */
						else {
							if ( cabj*cabj + sabj*sabj > 1.0e-8 ) ind1 = 2 ; else ind1 = 0 ;
					...
					if ( ipr == 0 ) ind2 = 1 ;
						...
						if ( ipr != j ) {
							if ( icon1[ipr] != jp1 ) ind2 = 2 ;
								...
								xi = fabs( cabj*cab[ipr] + sabj*sab[ipr] + salpj*salp[ipr] ) ;
								if ( ( xi < 0.999999 ) || ( fabs(bi[ipr]/b-1.) > 1.0e-6 ) ) ind2 = 2 ; else ind2 = 0 ;
							}
						} /* if ( ipr != j )*/
						else {
							if ( cabj*cabj + sabj*sabj > 1.0e-8 ) ind1 = 2 ; else ind1 = 0 ;
...
static void rdpat( void )
{
  ...
  char *igntp[5] =  { " MAJOR AXIS", "  MINOR AXIS",
      ...
      for ( kth = 1; kth <= nth ; kth++ ) {
			...
			if ( ifar != 1 ) {
				if ( (ethm2 <= 1.0e-20 ) && (ephm2 <= 1.0e-20 ) ) {
					...
					if ( fabs(dfaz) > fabs(dfaz2) ) dfaz = dfaz2;
    ...
    if ( iavp != 0 )
    {
      ...
      tmp3= fabs( dph* TA* (doubletype)( nph-1 )*( cos( tmp3)- cos( tmp4))) ;
      pint /= tmp3;
      tmp3 /= PI;
...
static void sfldsdouble( doubletype t, complex double *e )
{
	...
	if ( fabs( cph ) < 1.0e-10 ) cph = 0.0 ;
	if ( fabs( sph ) < 1.0e-10 ) sph = 0.0 ;
